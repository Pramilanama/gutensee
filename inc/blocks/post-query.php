...
global $gutensee_posts_query_excerpt_length;
$gutensee_posts_query_excerpt_length = 0;

function gutensee_posts_query_excerpt_length() {
    global $gutensee_posts_query_excerpt_length;
    return $gutensee_posts_query_excerpt_length;
}

function gutensee_render_post_query_callback($attributes) {
    global $gutensee_posts_query_excerpt_length;

    $paged = (get_query_var('paged')) ? get_query_var('paged') : 1;
    $order = isset($attributes['sortOrder']) ? $attributes['sortOrder'] : 'desc';
    $categories = isset($attributes['selectedCategories']) ? $attributes['selectedCategories'] : [];
    $post_count = isset($attributes['postCount']) ? $attributes['postCount'] : 5;
    $display_content = isset($attributes['displayContent']) ? $attributes['displayContent'] : false;
    $excerpt_limit = isset($attributes['excerptLimit']) ? $attributes['excerptLimit'] : 55;
    $read_more_text = isset($attributes['readMoreText']) ? $attributes['readMoreText'] : 'Read More';
    $display_tags = isset($attributes['displayTags']) ? $attributes['displayTags'] : false;
    ...
    $gutensee_posts_query_excerpt_length = $excerpt_limit;
    add_filter( 'excerpt_length', 'gutensee_posts_query_excerpt_length', 20 );
        ...
        if($attributes['poststyle']=='grid-layout'){
            ...
            foreach ($attributes['reorder'] as $index => $field) {
                switch ($field) {
                    ...
                    case 'Meta':
                        if(($attributes['displayAuthor']!=false) || ($attributes['displayDate']!=false) || ($attributes['displayCat']!=false) || ($attributes['displayComment']!=false)) {
                    ...
                    case 'Content':
                        if($attributes['displayPostContent']):
                            ...
                            if (  $attributes['disablebtn'] !=false &&  $attributes['displayPostContentRadio'] === 'excerpt'  ) {
                                ...
                                $btn_markup .='<a  href="'.esc_url( $post_link ).'" class="post-button more-link" target="'.$target.'">'.$attributes['buttonlabel'].'<i class="fas fa-arrow-right"></i></a>';
                            }

                            if ( $attributes['displayContent']==false) {
    ...
    if($attributes['hidepagination']==true){
        $list_items_markup .= '<div class="pagination">';
        if ($paged > 1) {
            $list_items_markup .= '<a href="' . get_pagenum_link($paged - 1) . '">Previous</a>';
        }
        $list_items_markup .= '<span>Page ' . $paged . ' of ' . $query->max_num_pages . '</span>';
        if ($paged < $query->max_num_pages) {
            $list_items_markup .= '<a href="' . get_pagenum_link($paged + 1) . '">Next</a>';
        }
        $list_items_markup .= '</div>';
    }
